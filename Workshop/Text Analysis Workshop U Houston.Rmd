...
3. So to improve on them, for each document $\displaystyle W$ do the following:
  + Go through each word $\displaystyle w$ in $\displaystyle W$
  ++ And for each topic $\displaystyle t$, compute two things: 
  +++ $\displaystyle p(t|W)$ = the proportion of words in document $\displaystyle W$ that are currently assigned to topic $\displaystyle t$, and
+++ $\displaystyle p(w|t)$ = the proportion of assignments to topic $\displaystyle t$ over all documents that come from this word $\displaystyle w$. 
...
(This explanation was taken from [here](https://wiki.ubc.ca/Course:CPSC522/Latent_Dirichlet_Allocation#cite_note-rcode-4))
...
```{r}
plot(cap_TM, type = "labels", labeltype = "frex") # or prob, lift score
```
 
If you want to see a sample of a specific topic:
   
```{r}
findThoughts(cap_TM, texts = texts(cap_corp)[docnames(dfm_toks_sub)], topics = 2)  
```
...
```
 ...
 What about connecting this info to our dfm and see if there are differences in the proportion topic 3 is addressed by each side. 
 
```{r}
 cap_prev <- data.frame(topic3 = cap_TM$theta[,3], docvars(data_capitalismm_basic))
 lmer_topic3 <- lmer(topic3 ~ (1 | album), data = cap_prev)
 dotplot(ranef(lmer_topic3, condVar = TRUE))
```
...
We can do something similar with the `stm` function directly. We just need to specify the functional form and add the document variables. 
 ...
 cap_prev_2 <- stm(data_capitalismm_basic, K = 10,
 prevalence = ~ year, 
 data = docvars(data_capitalismm_basic),
 seed = 1984)
 ```
 
 ```{r}
 cap_topics <- estimateEffect(c(3) ~ year, cap_prev_2, docvars(data_capitalismm_basic)) # You can commpare topic 2 and 3 by changin the DV to c(2,3). 
 plot(cap_topics, "year", labeltype = "prob", model = cap_prev_2)
 ```
...
data_capitalism_sub <- data_capitalism[data_capitalism$count_auth>9,]
...
We can also turn around the scaling and see where each word is positioned on the same left-right scale as the authors. Here is the "Eiffel Tower" of scaled words:
...
wscores <- data.frame(word = cap_wfish$features,
  ...
  ggplot(wscores, aes(score, offset, label = word)) +
    ...
    ggtitle("Estimated position of words",
            subtitle = "Nota: Parameter offset es proporcional a la frecuencia de cada palabra.")
```
...
```{r}
cap_parsed <- spacy_parse(data_capitalism$song_lyrics)
head(cap_parsed,25) 
```
...
```{r}
cap_parsed_dep <- spacy_parse(data_capitalism$song_lyrics, dependency = TRUE, entity = TRUE, lemma = FALSE, tag = TRUE)
...
How are words and authors connected? 
...
![Cosine similarity](http://blog.christianperone.com/wp-content/uploads/2013/09/cosinesimilarityfq1.png)
...
![Sentiment word embedding dimensions](https://www.tandfonline.com/na101/home/literatum/publisher/tandf/journals/content/hcms20/2018/hcms20.v012.i02-03/19312458.2018.1455817/20181105/images/large/hcms_a_1455817_f0002_oc.jpeg)
